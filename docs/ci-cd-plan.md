...
- **Only the latest runtime cut is kept.** Each push that changes the build inputs publishes a fresh `runtime-<hash>` (+ `runtime-x11-<hash>`) pair, so the Releases list would otherwise accumulate stale per-commit cuts. After publishing, `build-runtime.yml` runs `scripts/runtime-prune.sh` (headless matrix leg, push events only) to delete every `runtime-*` release and tag except the current hash's headless+x11 pair. The `v*` source releases are never touched. The prune is idempotent and 404-tolerant, so a parallel matrix leg or a re-run cannot delete the wrong cut.
